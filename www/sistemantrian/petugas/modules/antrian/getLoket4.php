...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ( $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' )) {
    ...
    $result_nomor = $mysqli->query("SELECT no_antrian FROM antrian WHERE tanggal='$hari_ini' AND loket='$loket' AND status='1' ORDER BY no_antrian DESC LIMIT 1")
    ...
    // fungsi query untuk menampilkan data dari tabel antrian
    $result_jumlah = $mysqli->query("SELECT count(ID) as jumlah FROM antrian WHERE tanggal='$hari_ini' AND loket='$loket' AND status='1'")
...
?>
    <div class="p-2 text-xs-center bg-cyan media-left media-middle">
        <h1 style="color:#fff"><?php echo $no_antrian; ?></h1>
    </div>
    <div class="p-2 media-body">
        <h5 class="cyan">LOKET 4</h5>
        <h5 class="text-bold-400"><?php echo number_format($jumlah_antrian); ?></h5>
        <progress class="progress progress-sm progress-cyan mt-1 mb-0" value="<?php echo $jumlah_antrian; ?>" max="100"></progress>
    </div>
<?php  
} else {
