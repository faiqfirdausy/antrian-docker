...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    ...
    $result = $mysqli->query("SELECT count(ID) as jumlah FROM antrian WHERE tanggal='$hari_ini' && layanan_antrian = '" . ANTRIAN_PERPUSTAKAAN_HUKUM . "'")
        or die('Ada kesalahan pada query tampil jumlah antrian: ' . $mysqli->error);
    $data = $result->fetch_assoc();
    $jumlah_antrian = $data['jumlah'];
    ?>
    Antrian Perpustakaan Hukum Sekarang: E<?php
